...
def get_hydrostatic_report(hs_data, cog, disp_tons, rho_water, grav):
    ...
    msg += build_line('Waterplane center (M)', hs_data['flotation_center'], precision=3)
    msg += build_line('Wet area (M**2)', hs_data['wet_surface_area'], precision=1)
    ...
    msg += build_line('Buoyancy center (M)', hs_data['buoy_center'], precision=3)
    msg += build_line('Center of gravity (M)', cog, precision=3)

    msg += hspace()
    msg += build_line('Draught (M)', hs_data['draught'], precision=3)  # TODO
    ...
    msg += build_line('Forward perpendicular (M)', hs_data['fp'], precision=2)

    msg += hspace()
    msg += build_line('Transversal metacentric radius (M)', hs_data['transversal_metacentric_radius'], precision=3)
    msg += build_line('Transversal metacentric height GMt (M)', hs_data['gm_x'], precision=3)
    msg += build_line('Longitudinal metacentric radius (M)', hs_data['longitudinal_metacentric_radius'], precision=3)
    msg += build_line('Longitudinal metacentric height GMl (M)', hs_data['gm_y'], precision=3)
